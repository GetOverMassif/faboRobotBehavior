...
{
    // std::cout << "!!rm_driver1 MoveJ_Callback!!" << std::endl;
    int res = 0;
    int i = 0;
    byte speed;
    float joint[6];
...
void Tool_DO_Callback(const rm_msgs::Tool_Digital_Output msg)
{
    ...
    {
        ROS_INFO("Before Set_Tool_DO_State.\n");
        res = Set_Tool_DO_State(msg.num, msg.state);
        ROS_INFO("After Set_Tool_DO_State.\n");
        if(res == 0)
        {
...
int main(int argc, char **argv)
{
    ...
    //subscriber
    MoveJ_Cmd = nh_.subscribe("/rm_driver1/MoveJ_Cmd", 10, MoveJ_Callback);
    MoveL_Cmd = nh_.subscribe("/rm_driver1/MoveL_Cmd", 10, MoveL_Callback);
    MoveC_Cmd = nh_.subscribe("/rm_driver1/MoveC_Cmd", 10, MoveC_Callback);
    JointPos_Cmd = nh_.subscribe("/rm_driver1/JointPos", 10, JointPos_Callback);
    Arm_DO_Cmd = nh_.subscribe("/rm_driver1/Arm_Digital_Output", 10, Arm_DO_Callback);
    Arm_AO_Cmd = nh_.subscribe("/rm_driver1/Arm_Analog_Output", 10, Arm_AO_Callback);
    Tool_DO_Cmd = nh_.subscribe("/rm_driver1/Tool_Digital_Output", 10, Tool_DO_Callback);
    Tool_AO_Cmd = nh_.subscribe("/rm_driver1/Tool_Analog_Output", 10, Tool_AO_Callback);
    Gripper_Cmd = nh_.subscribe("/rm_driver1/Gripper_Pick", 10, Gripper_Pick_Callback);
    Gripper_Set_Cmd = nh_.subscribe("/rm_driver1/Gripper_Set", 10, Gripper_Set_Callback);
    Emergency_Stop = nh_.subscribe("/rm_driver1/Emergency_Stop", 1, Stop_Callback);
    Joint_En = nh_.subscribe("/rm_driver1/Joint_Enable", 10, Joint_Enable_Callback);
    IO_Update = nh_.subscribe("/rm_driver1/IO_Update", 1, IO_Update_Callback);
...
/***** ********************************END****************************************/
    ...
    //publisher
    Joint_State = nh_.advertise<sensor_msgs::JointState>("/rm_driver1/joint_states", 300);
    Arm_IO_State = nh_.advertise<rm_msgs::Arm_IO_State>("/rm_driver1/Arm_IO_State", 1);
    Tool_IO_State = nh_.advertise<rm_msgs::Tool_IO_State>("/rm_driver1/Tool_IO_State", 1);
    Plan_State = nh_.advertise<rm_msgs::Plan_State>("/rm_driver1/Plan_State", 1);
